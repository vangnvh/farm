...
if(isset($_REQUEST['p']))
{
	...
	{
		$p = $_SESSION["employee_page"];
	}
}
$_SESSION["employee_page"] = $p;
...
{
	if(isset($_SESSION["employee_pages"]))
	{
		$ps = $_SESSION["employee_pages"];
	}
}
$_SESSION["employee_pages"] = $ps;	
...
for($i = 0; $i<count($items); $i++)
{
	...
	if(count($arr) == 2)
	{
		if($arr[0] == "search")
		{
			...
			$_SESSION["employee_search"] = $search;	
		}
		if($arr[0] == "p")
		{
			$p = urldecode($arr[1]);
...
if(isset($_SESSION["employee_search"]))
{
	$search = $_SESSION["employee_search"];
}
...
<section id="page-title">
	...
	<div class="container clearfix">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" ><a href="<?php echo URL;?><?php echo $lang; ?>/employeeform"><?php echo __('employees');?></a></li>
		</ol>
	</div>
...
<section id="content">
	...
	<div class="content-wrap">
		...
		<div class="container clearfix">
				...
				<div class="row">
					<div class="col-sm-8">
						<a class="button button-3d nomargin" href="<?php echo URL;?><?php echo $lang; ?>/employeeform/new" value="login"><?php echo __('New Employee');?></a>
					</div>
					<div class="col-sm-4">
				...
				<div class="table-responsive">
				  <table class="table table-bordered nobottommargin">
					...
					<?php
					$sql = "SELECT d1.id, d2.partner_code, d2.partner_name, d2.phone, d2.email, d2.address FROM hr_employee d1 LEFT OUTER JOIN res_partner d2 ON(d1.partner_id = d2.id) WHERE d1.status =0 AND d1.company_id='".$LOGIN_COMPANY_ID."'";
					if($search != "")
					{
					...
					<tbody>
						...
						<td nowrap="nowrap"><a href="<?php echo URL;?><?php echo $lang; ?>/employeeform/edit/<?php echo $employee_id; ?>"><?php echo __('Edit');?></a> / <a href="javascript:delLine('<?php echo $employee_id; ?>')"><?php echo __('Delete');?></a></td>
	...
	<script>
		function delLine(id)
		{
			...
			var _url = '<?php echo URL;?>includes/action.php?ac=delEmployee';
			...
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
					if(message == "OK")
					{
						...
						document.location.href ='<?php echo URL;?><?php echo $lang; ?>/employee?p=<?php echo $p?>';
					}
					else{
						alert(message);
					}
		...
		function doSearch(p)
		{
			...
			document.location.href ='<?php echo URL;?><?php echo $lang; ?>/employee?search=' + encodeURIComponent(search) + "&p=" + p;
		}
		function delRows()
		{
			var count = 0;
