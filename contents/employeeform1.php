...
<section id="page-title">
	...
	<div class="container clearfix">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item  active"><a href="<?php echo URL;?><?php echo $lang; ?>/employee"><?php echo __('Employees');?></a></li>
...
<section id="content">
	...
	<div class="content-wrap">
		...
		if($ac == "edit")
		{
			...
			if($employee_id != "")
			{
				$sql = "SELECT d1.id, d1.barcode, d2.partner_code, d2.partner_name, d2.phone, d2.vat, d2.email, d2.address, d1.partner_id FROM hr_employee d1 LEFT OUTER JOIN res_partner d2 ON(d1.partner_id = d2.id) WHERE d1.status =0 AND d1.id='".$employee_id."'";
				...
				$sql = "SELECT d1.user_id, d2.user_name, d2.password FROM res_user_company d1 LEFT OUTER JOIN res_user d2 ON(d1.user_id = d2.id) WHERE d1.status =0 AND d1.employee_id='".$employee_id."'";
				$result = pg_exec($db, $sql);
				$numrows = pg_numrows($result);
				if($numrows>0)
				{
		...
		<div class="col_full nobottommargin">
			<a class="button button-3d nomargin" href="javascript:saveCustomer()" value="register"><?php echo __('Save');?></a> <a class="button button-3d button-black nomargin" href="<?php echo URL;?><?php echo $lang; ?>/employee" value="register"><?php echo __('Back');?></a> 
		</div>
		<br>
		<form id="frmRegister" name="frmRegister" class="nobottommargin" >
					...
					<div class="row">
							<div class="col-sm-2">
								<label for="editvat"><?php echo __('Employee Code');?> <span style="color:red"><small >*</small></span></label>
								<input type="text" id="editpartner_code" name="editpartner_code" value="<?php echo $partner_code; ?>" class="form-control" maxlength = "64"  />
							</div>
							<div class="col-sm-8">
								<label for="editcompany_name"><?php echo __('Employee Name');?> <span style="color:red"><small >*</small></span></label>
							...
							<div class="col-sm-2">
								<label for="editvat"><?php echo __('Barcode');?></label>
								<input type="text" id="editbarcode" name="editbarcode" value="<?php echo $barcode; ?>" class="form-control" maxlength = "64"  />
							</div>
					</div>
	...
	<script>
		...
		function saveCustomer()
		{
			...
			ctr = document.frmRegister.editbarcode;
			var barcode = ctr.value;
			...
			var _url = '<?php echo URL;?>includes/action.php?ac=saveEmployee';
			...
			_url = _url + '&employee_id=<?php echo $employee_id;?>';
			...
			_url = _url + '&barcode=' + encodeURIComponent(barcode);
			...
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
					if(message == "OK")
					{
						...
						document.location.href ='<?php echo URL;?><?php echo $lang; ?>/employee';
					}
					else{
						alert(message);
					}
	...
	<script>
		...
		{
			var _url = '<?php echo URL;?>includes/action.php?ac=addEmployee&user_id=' + id;
			...
			loadPage('pnListUser', _url, function(status, message)
			{
				if(status== 0)
				{
					...
					{
						document.location.href ='<?php echo URL;?><?php echo $lang; ?>/employeeform/edit/' + message;
					}else{
						alert(message);
					}
				}
