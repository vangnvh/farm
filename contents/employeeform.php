...
<section id="content">
	...
	<div class="content-wrap">
		...
		if($uri !='/' && $uri != '')
		{
			...
			if($id != "")
			{
				$sql = "SELECT d1.id, d2.partner_code, d2.partner_name, d2.phone, d2.vat, d2.email, d2.address, d1.partner_id , d2.bank_no, d2.bank_name FROM hr_employee d1 LEFT OUTER JOIN res_partner d2 ON(d1.partner_id = d2.id) WHERE d1.status =0 AND d1.id='".$id."'";
		...
		<div class="col_full nobottommargin">
			<a class="button button-3d nomargin" href="javascript:saveLine()" value="register"><?php echo __('Save');?></a> <a class="button button-3d button-black nomargin" href="<?php echo URL;?><?php echo $lang; ?>/employee" value="register"><?php echo __('Back');?></a> 
		</div>
		<br>
	...
	<script>
		...
		function saveLine()
		{
			...
			_url = _url + '&employee_id=<?php echo $id;?>';
		...
		function loadUsers()
		{
			var _url = '<?php echo URL;?>includes/user_employee.php?ac=view&rel_id=<?php echo $id;?>';
		...
		function addUser()
		{
			var _url = '<?php echo URL;?>includes/user_employee.php?ac=addLine&rel_id=<?php echo $id;?>';
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
		...
		{
			var _url = '<?php echo URL;?>includes/user_employee.php?ac=saveLine';
			_url = _url + '&id=' + id;
			_url = _url + '&name=' + name;
		...
		function delUser(id)
		{
			...
			var _url = '<?php echo URL;?>includes/user_employee.php?ac=delLine';
			_url = _url + '&id=' + id;
