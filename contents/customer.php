...
<section id="content">
	...
	<div class="content-wrap">
		...
		<div class="container clearfix">
				...
				<div class="row">
					<div class="col-sm-8">
						<a class="button button-3d nomargin" href="<?php echo URL;?><?php echo $lang; ?>/customerform/new" value="login"><?php echo __('New Customer');?></a>
					</div>
					<div class="col-sm-4">
				...
				<div class="table-responsive">
				  <table class="table table-bordered nobottommargin">
					...
					<?php
					$sql = "SELECT d1.id, d2.partner_code, d2.partner_name, d2.phone, d2.email, d2.address, d2.barcode FROM customer d1 LEFT OUTER JOIN res_partner d2 ON(d1.partner_id = d2.id) WHERE d1.status =0 AND d1.company_id='".$LOGIN_COMPANY_ID."'";
					if($search != "")
					{
					...
					<tbody>
					  <tr>
						...
						<td nowrap="nowrap"><a href="<?php echo URL;?><?php echo $lang; ?>/customerform/edit/<?php echo $employee_id; ?>"><?php echo __('Edit');?></a> / <a href="javascript:delCustomer('<?php echo $employee_id; ?>')"><?php echo __('Delete');?></a></td>
	...
	<script>
			...
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
					if(message == "OK")
					{
						...
						document.location.href ='<?php echo URL;?><?php echo $lang; ?>/customer?p=<?php echo $p?>';
					}
					else{
						alert(message);
					}
		...
		function doSearch(p)
		{
			...
			document.location.href ='<?php echo URL;?><?php echo $lang; ?>/customer?search=' + encodeURIComponent(search) + "&p=" + p;
		}
		function delRows()
		{
			var count = 0;
