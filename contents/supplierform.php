...
<section id="page-title">
	...
	<div class="container clearfix">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" ><a href="<?php echo URL;?><?php echo $lang; ?>/supplier"><?php echo __('Suppliers');?></a></li>
...
<section id="content">
	...
	<div class="content-wrap">
		...
		if($uri !='/' && $uri != '')
		{
			...
			if($supplier_id != "")
			{
				$sql = "SELECT d1.id, d2.partner_code, d2.partner_name, d2.phone, d2.vat, d2.email, d2.address, d1.partner_id , d2.bank_no, d2.bank_name FROM supplier d1 LEFT OUTER JOIN res_partner d2 ON(d1.partner_id = d2.id) WHERE d1.status =0 AND d1.id='".$supplier_id."'";
		...
		<div class="col_full nobottommargin">
			<a class="button button-3d nomargin" href="javascript:saveSupplier()" value="register"><?php echo __('Save');?></a> <a class="button button-3d button-black nomargin" href="<?php echo URL;?><?php echo $lang; ?>/supplier" value="register"><?php echo __('Back');?></a> 
		</div>
		<br>
	...
	<script>
		...
		function saveSupplier()
		{
			...
			var _url = '<?php echo URL;?>includes/action.php?ac=saveSupplier';
			...
			_url = _url + '&supplier_id=<?php echo $supplier_id;?>';
			...
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
					if(message.length == 36)
					{
						...
						document.location.href ='<?php echo URL;?><?php echo $lang; ?>/supplierform/edit/' + message;
					}else if(message == "CODE_AVAIBLE")
					{
						alert("Mã nhà cung cấp đang tồn tại");
						return;
					}
					else{
						alert(message);
		...
		{
			var _url = '<?php echo URL;?>includes/mrp_location.php?ac=view&rel_id=<?php echo $supplier_id;?>';
		...
		{
			var _url = '<?php echo URL;?>includes/mrp_location.php?ac=addLine&rel_id=<?php echo $supplier_id;?>';
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
