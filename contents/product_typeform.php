...
<section id="page-title">
	...
	<div class="container clearfix">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" ><a href="<?php echo URL;?><?php echo $lang; ?>/product_type"><?php echo __('Product Type');?></a></li>
...
<section id="content">
	...
	<div class="content-wrap">
		...
		if($uri !='/' && $uri != '')
		{
			...
			if($id != "")
			{
				$sql = "SELECT d1.id, d1.name, d1.description, d1.factor FROM product_type d1 WHERE d1.status =0 AND d1.id='".$id."'";
		...
		<form id="frmRegister" name="frmRegister" class="nobottommargin" >
					...
					<div class="row">
							...
							<div class="col-sm-2">
								<label for="editfactor"><?php echo __('%');?> <span style="color:red"><small ></small></span></label>
								<input type="text" autocomplete="off" id="editfactor" name="editfactor" value="<?php echo $factor; ?>" class="form-control" maxlength = "250" />
							</div>
	...
	<script>
		...
		function saveLine()
		{
			...
			ctr = document.frmRegister.editfactor;
			var factor = ctr.value;
			if(factor == "")
			{
				factor = 0;
			}
			...
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
					if(message == "OK")
					{
						document.location.href ='<?php echo URL;?><?php echo $lang; ?>/product_type';
					}
					else{
						alert(message);
					}
