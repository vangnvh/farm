...
if(isset($_REQUEST['p']))
{
	...
	if(isset($_SESSION["payment_type_page"]))
	{
		$p = $_SESSION["payment_type_page"];
	}
}
...
{
	if(isset($_SESSION["payment_type_pages"]))
	{
		$ps = $_SESSION["payment_type_pages"];
	}
}
$_SESSION["payment_type_pages"] = $ps;	
...
for($i = 0; $i<count($items); $i++)
{
	...
	if(count($arr) == 2)
	{
		if($arr[0] == "search")
		{
			...
			$_SESSION["payment_type_search"] = $search;	
		}
		if($arr[0] == "p")
		{
			$p = urldecode($arr[1]);
			$_SESSION["payment_type_page"] = $p;
		}
	}
}
if(isset($_SESSION["payment_type_search"]))
{
	$search = $_SESSION["payment_type_search"];
}
...
<section id="page-title">
	...
	<div class="container clearfix">
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" ><a href="<?php echo URL;?><?php echo $lang; ?>/paymentform"><?php echo __('Payment Category');?></a></li>
		</ol>
	</div>
...
<section id="content">
	...
	<div class="content-wrap">
		...
		<div class="container clearfix">
				...
				<div class="row">
					<div class="col-sm-8">
						<a class="button button-3d nomargin" href="<?php echo URL;?><?php echo $lang; ?>/paymentform/new" value="login"><?php echo __('New Payment');?></a>
					</div>
					<div class="col-sm-4">
				...
				<div class="table-responsive">
				  <table class="table table-bordered nobottommargin">
					...
					<?php
					$sql = "SELECT d1.id, d1.payment_name FROM res_payment d1 WHERE d1.status =0 AND d1.company_id='".$LOGIN_COMPANY_ID."'";
					if($search != "")
					{
						$sql = $sql." AND ( d1.payment_name ILIKE '%".str_replace("'", "''", $search)."%')";
					}
					$arr = paging($sql, $p, $ps, "d1.payment_name ASC");
					...
					<tbody>
						...
						<td><?php echo $name; ?></td>
						<td style="width:40px" nowrap="nowrap"><a href="<?php echo URL;?><?php echo $lang; ?>/paymentform/edit/<?php echo $id; ?>"><?php echo __('Edit');?></a> / <a href="javascript:delLine('<?php echo $id; ?>')"><?php echo __('Delete');?></a></td>
	...
	<script>
		function delLine(id)
		{
			...
			var _url = '<?php echo URL;?>includes/action.php?ac=delPayment';
			...
			loadPage('gotoTop', _url, function(status, message)
			{
				if(status== 0)
				{
					if(message == "OK")
					{
						...
						document.location.href ='<?php echo URL;?><?php echo $lang; ?>/payment?p=<?php echo $p?>';
					}
					else{
						alert(message);
					}
		...
		function doSearch(p)
		{
			...
			document.location.href ='<?php echo URL;?><?php echo $lang; ?>/payment?search=' + encodeURIComponent(search) + "&p=" + p;
		}
		function delRows()
		{
			var count = 0;
